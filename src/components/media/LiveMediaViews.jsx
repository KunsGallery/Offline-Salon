...
export function PdfPageCanvas({ url, pageNumber }) {
  ...
  useEffect(() => {
    ...
    pdfDocument.getPage(pageNumber).then((page) => {
      ...
      const scale = Math.min((host.clientWidth - 16) / base.width, (host.clientHeight - 16) / base.height);
  ...
  }, [layoutVersion, pageNumber, pdfDocument]);
  return <div className={`salon-pdf-canvas ${status}`} ref={hostRef}>{status === 'loading' ? <p>페이지 준비 중…</p> : null}{status === 'error' ? <p>PDF 페이지를 표시하지 못했습니다.</p> : null}<canvas ref={canvasRef} /></div>;
}
...
export function ArtworkHostView({ session, artwork, responses }) {
  ...
  return <main className={`artwork-host-stage trophies ${phase === 'reveal' ? 'revealed' : ''}`}><header><p className="eyebrow">{phase === 'reveal' ? 'ARTWORK REVEAL' : 'TITLE VOTE'}</p><span>{responses.length} TITLES</span></header><div className="artwork-mini"><img src={artwork.imageUrl} alt="작품" />{phase === 'reveal' ? <div><strong>{artwork.title || '제목 미정'}</strong><span>{artwork.artist || '작가 미정'}</span></div> : null}</div><section className="title-trophy-board">{sorted.map((response, index) => { const likes = Number(response.likes || 0); return <article key={response.id} style={{ '--title-scale': Math.min(1.32, 1 + likes * 0.055), '--gold': Math.min(1, likes / 6) }} className={index === 0 && likes ? 'winner' : ''}><span>{index === 0 && likes ? '★' : '◇'}</span><h2>{safeJoin(response.value)}</h2><b>♥ {likes}</b></article>; })}</section></main>;
}
...
export function ArtworkParticipantView({ artwork, phase, responses, myResponse, onSubmit, onLike, participantId }) {
  ...
  return <main className="media-participant artwork-voting"><p className="eyebrow">{phase === 'reveal' ? 'ARTWORK REVEAL' : 'VOTE FOR A TITLE'}</p><h1>{phase === 'reveal' ? artwork.title || '작품 정보' : '마음에 드는 제목을 골라보세요.'}</h1>{phase === 'reveal' ? <p>{artwork.artist}<br />{artwork.description}</p> : null}<div className="mobile-title-list">{sorted.map((response) => <button key={response.id} disabled={phase !== 'vote' || response.participantId === participantId} onClick={() => onLike(response)}><span>{safeJoin(response.value)}</span><b>♥ {response.likes || 0}</b></button>)}</div></main>;
}
