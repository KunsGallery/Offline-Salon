...
export function ArtworkParticipantView({ artwork, phase, responses, myResponse, onSubmit, onLike, participantId }) {
  ...
  return <main className="media-participant artwork-voting"><p className="eyebrow">{phase === 'reveal' ? 'ARTWORK REVEAL' : 'VOTE FOR A TITLE'}</p><h1>{phase === 'reveal' ? '작품 정보가 공개되었습니다.' : '마음에 드는 제목을 골라보세요.'}</h1>{phase === 'reveal' ? <p>앞 화면에서 실제 작품 정보를 확인해 주세요.</p> : <p className="vote-guide">내가 낸 제목에는 투표할 수 없어요. 다른 참여자의 제목을 골라주세요.</p>}<div className="mobile-title-list">{sorted.map((response) => { const isOwn = response.participantId === participantId; return <button className={isOwn ? 'own-title' : ''} key={response.id} disabled={phase !== 'vote' || isOwn} onClick={() => onLike(response)}><span>{safeJoin(response.value)}{isOwn ? <small>내 제목 · 본인 투표 불가</small> : null}</span><b>♥ {response.likes || 0}</b></button>; })}</div></main>;
}
