...
function stageName(stage) {
  ...
  if (stage?.mode === 'image') return '이미지 발표';
  if (stage?.mode === 'artwork') return stage.phase === 'collect' ? '작품 제목 수집' : stage.phase === 'vote' ? '작품 제목 투표' : '작품 제목 공개';
  if (stage?.mode === 'gallery') return '결과 갤러리';
  if (stage?.mode === 'exhibition-grape') return stage.view === 'person' ? '참여자 전시 포도' : stage.view === 'collective' ? '전체 전시 포도밭' : '실시간 전시 카운터';
  if (stage?.mode === 'lobby') return '참여자 대기방';
  return '일반 질문';
}

export default function AdminLiveConsole({ session, questions = [], activeQuestion, responses, allResponses = [], participants, hostUrl, showPanel = true }) {
  ...
  const showGrapeActivity = () => realtime.updateSession(session.id, { currentQuestionId: null, stage: { mode: 'exhibition-grape', view: 'live', page: 1, blackout: false }, status: 'live' });
    ...
    {showPanel ? <section className="live-console-grid">
      <article className="panel stage-preview-card">
        ...
        <div className={`stage-preview stage-${stage.mode} ${stage.blackout ? 'blackout' : ''}`}>
          {stage.blackout ? <strong>화면 가림</strong> : stage.mode === 'pdf' && deck ? <><PdfPageCanvas url={deck.fileUrl} pageNumber={page} fitMode={stage.fitMode} zoom={stage.zoom} compact /><span>{page} / {deck.pageCount}</span></> : stage.mode === 'image' && artwork ? <><img src={artwork.imageUrl} alt="현재 이미지" /><div><strong>{artwork.displayTitle || artworkSecret.title || '이미지 표시 중'}</strong><span>앞 화면과 참여자 화면에 표시됩니다.</span></div></> : stage.mode === 'artwork' && artwork ? <><img src={artwork.imageUrl} alt="현재 작품" /><div><strong>{artwork.adoptedTitle || artworkSecret.title || '지난 모임 이미지 활동'}</strong><span>{visibleResponses.length}개 제목 도착</span></div></> : stage.mode === 'gallery' ? <div><strong>참여 결과 갤러리</strong><span>{galleryData.participantResults.length}명 · 결과 {galleryData.resultCount}개 · 이미지 {galleryData.mediaItems.length}개</span></div> : stage.mode === 'lobby' ? <div><strong>참여자 대기방</strong><span>{participants.length}명이 테이블에 앉았습니다.</span></div> : <div><strong>{activeQuestion?.title || '참여자 대기방'}</strong><span>{visibleResponses.length}개 응답 · {participants.length}명 접속</span></div>}
        </div>
      ...
      <article className="panel live-operation-card">
        ...
        <div className="operation-block"><span>화면 모드</span><button className="btn" disabled={busy} onClick={() => run(showLobby)}>대기방</button><button className="btn" disabled={busy || !(session.artworks || []).length} onClick={() => run(showGrapeActivity)}>전시 포도 시작</button><button className="btn" disabled={busy || !galleryReady} title={galleryReady ? '' : '갤러리에 포함된 결과나 이미지가 없습니다.'} onClick={() => run(showGallery)}>결과 갤러리 ({galleryData.resultCount + galleryData.mediaItems.length})</button><button className={`btn ${stage.blackout ? 'primary' : ''}`} disabled={busy} onClick={() => run(() => setView({ blackout: !stage.blackout }))}>{stage.blackout ? '화면 다시 표시' : '화면 잠시 가리기'}</button></div>
