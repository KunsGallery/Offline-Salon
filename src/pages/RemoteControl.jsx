...
import { findArtworkActivityQuestion } from '../lib/artworkActivity';
...
export default function RemoteControl() {
  ...
  const setGalleryPosition = (next) => realtime.updateSession(session.id, { stage: { ...stage, page: Math.min(Math.max(1, galleryScrollSteps), Math.max(1, next)), blackout: false } });
  ...
  const galleryData = buildResultGallery(session, session.questions || [], allReviewResponses, Object.values(session.participants || {}));
  ...
  if (adoptedReviewResponse?.questionId) reviewQuestionIds.add(adoptedReviewResponse.questionId);
  ...
  return <main className="remote-control remote-v2" style={sessionThemeStyle(session)}>
    ...
    <ExhibitionGrapeRemotePanel session={session} participants={participants} busy={busy} run={run} />
