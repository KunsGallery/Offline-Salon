...
import { resolveExhibitionReferences } from '../lib/exhibitionCatalog';
...
export default function AdminSession() {
  ...
  const checkinUrl = `${window.location.origin}/checkin/${sessionId}`;
  ...
  return (
    <main className="page-shell admin-session" style={sessionThemeStyle(session)}>
      ...
      <nav className="admin-workspace-tabs" aria-label="관리자 작업 구역">
        ...
        <button className={section === 'checkin' ? 'active' : ''} onClick={() => setSection('checkin')}><span>03</span>입장 체크</button>
        <button className={section === 'engagement' ? 'active' : ''} onClick={() => setSection('engagement')}><span>04</span>참여 현황</button>
        <button className={section === 'access' ? 'active' : ''} onClick={() => setSection('access')}><span>05</span>접속·QR</button>
      </nav>

      {section === 'setup' ? <section className="admin-workspace stack gap-lg"><div className="setup-summary"><div><span>{hasSessionModule(session, 'exhibition-grape') ? '참고 전시' : '갤러리 이미지'}</span><strong>{hasSessionModule(session, 'exhibition-grape') ? resolveExhibitionReferences(session).length : session.artworks?.length || 0}</strong></div><div><span>PDF</span><strong>{session.decks?.length || 0}</strong></div><div><span>참여 활동</span><strong>{questions.filter((item) => !item.internal).length}</strong></div><div><span>마지막 저장</span><strong>{formatDateTime(session.updatedAt)}</strong></div></div><SessionEditor session={session} /><SessionMediaStudio session={session} questions={questions} /></section> : null}
      ...
      {section === 'checkin' ? <section className="admin-workspace"><CheckinManager session={session} /></section> : null}
      ...
      {section === 'access' ? <section className="admin-workspace access-workspace"><section className="panel"><div className="panel-header"><div><p className="eyebrow">JOIN & CONTROL</p><h2>참여자·관리자 접속</h2><p className="muted">참여자, 리모컨, 입장 체크 화면을 각각 별도 URL로 엽니다.</p></div></div><div className="admin-qr-grid"><QRJoinCard sessionId={sessionId} /><QRJoinCard url={remoteUrl} title="관리자 휴대폰으로 스캔해 리모컨을 여세요" /><QRJoinCard url={checkinUrl} title="스태프 체크인 화면을 여세요" /></div></section><section className="panel readiness-card"><p className="eyebrow">EVENT CHECK</p><h2>행사 전 점검</h2><ul><li className={session.status === 'live' ? 'done' : ''}>세션 라이브 상태</li><li className={activeQuestion ? 'done' : ''}>활성 질문 준비</li><li className={(session.artworks?.length || session.decks?.length) ? 'done' : ''}>작품 또는 PDF 등록</li><li className={participants.length ? 'done' : ''}>참여자 연결 테스트</li><li className={(session.checkinApplications?.length || 0) ? 'done' : ''}>체크인 명단 준비</li></ul><button className="btn" onClick={() => window.open(hostUrl, '_blank', 'noopener,noreferrer')}>Host 화면 점검</button><button className="btn" onClick={() => window.open(checkinUrl, '_blank', 'noopener,noreferrer')}>체크인 화면 점검</button><button className="btn primary" onClick={() => window.open(remoteUrl, '_blank', 'noopener,noreferrer')}>리모컨 점검</button></section></section> : null}
    </main>
  );
}
